...
function identifyBehavioralPatterns(subscores) {
  const patterns = []
  const { low, high } = subscores.metadata.thresholds

  const { presentAwareness, nonreactiveObserving, responseFlexibility, internalStateAwareness, emotionalInsight } =
    subscores

  // Calculate averages for compound checks
  const avg =
    (presentAwareness + nonreactiveObserving + responseFlexibility + internalStateAwareness + emotionalInsight) / 5
  const range =
    Math.max(presentAwareness, nonreactiveObserving, responseFlexibility, internalStateAwareness, emotionalInsight) -
    Math.min(presentAwareness, nonreactiveObserving, responseFlexibility, internalStateAwareness, emotionalInsight)

  // === INTEGRATION PATTERNS (all high) ===

  if (
    presentAwareness > 3.5 &&
    nonreactiveObserving > 3.5 &&
    responseFlexibility > 3.5 &&
    internalStateAwareness > 3.5 &&
    emotionalInsight > 3.5
  ) {
    patterns.push('exceptionalMindfulness')
  } else if (
    presentAwareness > 3.0 &&
    nonreactiveObserving > 3.0 &&
    responseFlexibility > 3.0 &&
    internalStateAwareness > 3.0 &&
    emotionalInsight > 3.0
  ) {
    patterns.push('strongMindfulnessIntegration')
  } else if (
    [presentAwareness, nonreactiveObserving, responseFlexibility, internalStateAwareness, emotionalInsight].filter(
      (s) => s > 3.0,
    ).length === 4 &&
    avg >= 2.8
  ) {
  ...
  if (
    presentAwareness < 2.0 &&
    nonreactiveObserving < 2.0 &&
    responseFlexibility < 2.0 &&
    internalStateAwareness < 2.0 &&
    emotionalInsight < 2.0
  ) {
    ...
    presentAwareness < 2.3 &&
    nonreactiveObserving < 2.3 &&
    responseFlexibility < 2.3 &&
    internalStateAwareness < 2.3 &&
  ...
  if (range < 0.5 && avg >= 2.3 && avg <= 3.0) {
    ...
    avg >= 2.5 &&
    avg <= 3.0 &&
    presentAwareness >= 2.5 &&
    nonreactiveObserving >= 2.5 &&
    responseFlexibility >= 2.5 &&
  ...
  if (presentAwareness > 3.0 && emotionalInsight < 2.3 && nonreactiveObserving < 3.0 && internalStateAwareness < 3.0) {
  ...
  if (
    presentAwareness > 2.8 &&
    nonreactiveObserving > 2.8 &&
    responseFlexibility > 2.8 &&
    internalStateAwareness > 2.8 &&
    emotionalInsight < 2.3
  ) {
  ...
  const scores = [presentAwareness, nonreactiveObserving, responseFlexibility, internalStateAwareness, emotionalInsight]
...
function identifyNotablePattern(subscores) {
  ...
  if (hasLargeGap(emotionalInsight, presentAwareness) && emotionalInsight > 3.0 && presentAwareness < 2.3) {
    return 'intellectualVsRealtime'
  }

  // 4. Observation without action
  if (
    hasLargeGap(nonreactiveObserving, responseFlexibility) &&
    nonreactiveObserving > 3.0 &&
    responseFlexibility < 2.3
  ) {
  ...
  if (hasLargeGap(emotionalInsight, internalStateAwareness) && emotionalInsight > 3.0 && internalStateAwareness < 2.3) {
  ...
  if (hasLargeGap(presentAwareness, emotionalInsight) && presentAwareness > 3.0 && emotionalInsight < 2.3) {
